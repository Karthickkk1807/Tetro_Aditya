...
namespace TetroONE.Controllers
{
    ...
    public class InventoryController : BaseController
    {
        ...
        [HttpGet]
        [Route("GetAssetManagementDetails")]
        public IActionResult GetAssetManagementDetails(int FranchiseId, int AssetManagementId, int TypeId, int ClientId, string Type)
        {
            GetAssetManagementDetails request = new GetAssetManagementDetails()
            {
                LoginUserId = Convert.ToInt32(User.Claims.FirstOrDefault(c => c.Type == ClaimTypes.NameIdentifier)?.Value),
                FranchiseId = FranchiseId,
                AssetManagementId = AssetManagementId == 0 ? null : AssetManagementId,
                TypeId = TypeId,
                ClientId = ClientId,
                Type = Type
            };

            response = GenericTetroONE.GetData(_connectionString, "[dbo].[USP_GetAssetManagementDetails]", request);
            return Json(response);
        }


        [HttpPost]
        [Route("InsertUpdateNonMovableAssetDetails")]
        public async Task<IActionResult> InsertUpdateNonMovableAssetDetails()
        {
            ...
            try
            {
                AssetNonMovableStatic AssetNonMovableStatic = JsonConvert.DeserializeObject<AssetNonMovableStatic>(Request.Form["AssetNonMovableStatic"]);

                InsertUpdateAssetNonMovableDetails request = new InsertUpdateAssetNonMovableDetails()
                {
                    LoginUserId = Convert.ToInt32(User.Claims.FirstOrDefault(c => c.Type == ClaimTypes.NameIdentifier)?.Value),
                    AssetNon_MovableId = AssetNonMovableStatic.AssetNon_MovableId,
                    FranchiseId = AssetNonMovableStatic.FranchiseId,
                    MachineTypeId = AssetNonMovableStatic.MachineTypeId,
                    MachineName = AssetNonMovableStatic.MachineName,
                    DepartmentId = AssetNonMovableStatic.DepartmentId,
                    PurchaseDate = AssetNonMovableStatic.PurchaseDate,
                    NoOfAssets = AssetNonMovableStatic.NoOfAssets,
                    Amount = AssetNonMovableStatic.Amount,
                    NextServiceDate = AssetNonMovableStatic.NextServiceDate,
                    DepreciationPercentage = AssetNonMovableStatic.DepreciationPercentage,
                    Comments = AssetNonMovableStatic.Comments,

                    TVP_AttachmentDetails = dtattachment
                };

                if (AssetNonMovableStatic.AssetNon_MovableId > 0)
                    response = GenericTetroONE.ExecuteReturnData(_connectionString, "[dbo].[USP_UpdateNon_MovableAssetDetails]", request);
                else
                    response = GenericTetroONE.ExecuteReturnData(_connectionString, "[dbo].[USP_InsertNon_MovableAssetDetails]", request, "AssetNon_MovableId");
        ...
        [HttpPost]
        [Route("InsertUpdateAssetMovableDetails")]
        public async Task<IActionResult> InsertUpdateAssetMovableDetails()
        {
            IFormFileCollection file = Request.Form.Files;
            List<AttachmentTable> lstattachment = new List<AttachmentTable>();
            DataTable dtattachment = new DataTable();
            ...
            try
            {
                AssetMovableStatic AssetMovableStatic = JsonConvert.DeserializeObject<AssetMovableStatic>(Request.Form["AssetNonMovableStatic"]);

                InsertUpdateAssetMovableDetails request = new InsertUpdateAssetMovableDetails()
                {
                    LoginUserId = Convert.ToInt32(User.Claims.FirstOrDefault(c => c.Type == ClaimTypes.NameIdentifier)?.Value),
                    MovableAssetId = AssetMovableStatic.MovableAssetId,
                    FranchiseId = AssetMovableStatic.FranchiseId,
                    MovableAssetTypeId = AssetMovableStatic.MovableAssetTypeId,
                    TypeName = AssetMovableStatic.TypeName,
                    DepartmentId = AssetMovableStatic.DepartmentId,
                    PurchaseDate = AssetMovableStatic.PurchaseDate,
                    NoOfAssets = AssetMovableStatic.NoOfAssets,
                    Amount = AssetMovableStatic.Amount,
                    NextServiceDate = AssetMovableStatic.NextServiceDate,
                    DepreciationPercentage = AssetMovableStatic.DepreciationPercentage,
                    Comments = AssetMovableStatic.Comments,

                    TVP_AttachmentDetails = dtattachment
                };

                if (AssetMovableStatic.MovableAssetId > 0)
                    response = GenericTetroONE.ExecuteReturnData(_connectionString, "[dbo].[USP_UpdateMovableAssetDetails]", request);
                else
                    response = GenericTetroONE.ExecuteReturnData(_connectionString, "[dbo].[USP_InsertMovableAssetDetails]", request, "MovableAssetId");
        ...
        [HttpGet]
        [Route("DeleteMovable")]
        public IActionResult DeleteMovable(int MovableAssetId)
        {
            DeleteMovable request = new DeleteMovable()
            {
                LoginUserId = Convert.ToInt32(User.Claims.FirstOrDefault(c => c.Type == ClaimTypes.NameIdentifier)?.Value),
                MovableAssetId = MovableAssetId,
            };

            response = GenericTetroONE.GetData(_connectionString, "[dbo].[USP_DeleteMovableAssetDetails]", request);
            return Json(response);
        }

        [HttpGet]
        [Route("DeleteNon_Movable")]
        public IActionResult DeleteNon_Movable(int AssetNon_MovableId)
        {
            DeleteNon_Movable request = new DeleteNon_Movable()
            {
                LoginUserId = Convert.ToInt32(User.Claims.FirstOrDefault(c => c.Type == ClaimTypes.NameIdentifier)?.Value),
                AssetNon_MovableId = AssetNon_MovableId,
            };

            response = GenericTetroONE.GetData(_connectionString, "[dbo].[USP_DeleteNon_MovableAssetDetails]", request);
            return Json(response);
        }

        [HttpGet]
        [Route("GetDepreciationValueDetails_AssetManagement")]
        public IActionResult GetDepreciationValueDetails_AssetManagement(DateTime PurchaseDate, decimal Amount, decimal DepreciationPercentage)
        {
            GetDepreciationValueDetails_AssetManagement request = new GetDepreciationValueDetails_AssetManagement()
            {
                LoginUserId = Convert.ToInt32(User.Claims.FirstOrDefault(c => c.Type == ClaimTypes.NameIdentifier)?.Value),
            ...
            response = GenericTetroONE.GetData(_connectionString, "[dbo].[USP_GetDepreciationValueDetails_AssetManagement]", request);
            return Json(response);
        }

        [HttpGet]
        [Route("USP_DD_GetProductDetails_ByProductId_ProductionPlan")]
        public IActionResult USP_DD_GetProductDetails_ByProductId_ProductionPlan(int FranchiseId, int ProductId)
        {
            USP_DD_GetProductDetails_ByProductId_ProductionPlan request = new USP_DD_GetProductDetails_ByProductId_ProductionPlan()
            {
                LoginUserId = Convert.ToInt32(User.Claims.FirstOrDefault(c => c.Type == ClaimTypes.NameIdentifier)?.Value),
                FranchiseId = FranchiseId,
                ProductId = ProductId,
            ...
            response = GenericTetroONE.GetData(_connectionString, "[dbo].[USP_DD_GetProductDetails_ByProductId_ProductionPlan]", request);
            return Json(response);
        }
