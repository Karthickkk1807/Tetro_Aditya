...
{
	public class PDF_DPO
	{

		public byte[] PurchaseOrderPrintNewDPO(PurchaseOrderPrintDPO data, int numberOfCopies)
		{
				...
				using (MemoryStream stream = new MemoryStream())
				{
					using (PdfWriter writer = new PdfWriter(stream))
					{
						using (PdfDocument pdf = new PdfDocument(writer))
						{
							...
							CompanyName.AddCell(new Cell(1, 2).SetBackgroundColor(HeaderBackgroundColor).SetPaddingLeft(80).SetBorder(Border.NO_BORDER).Add(new Paragraph(data.CompanyName).SetFont(kabrioBoldFont).SetFontSize(14).SetTextAlignment(TextAlignment.CENTER)));
							...
							ModuleTable.AddCell(new Cell().SetBorder(Border.NO_BORDER).SetPaddingTop(5).Add(new Paragraph("Purchase Order").SetFont(kabrioBoldFont).SetFontSize(15).SetTextAlignment(TextAlignment.CENTER).SetMarginTop(-5)));
							...
							CustomerDetaisTable.AddCell(new Cell(1, 2).SetBorderBottom(new SolidBorder(1)).SetBorder(Border.NO_BORDER).SetPadding(0).Add(new Paragraph("Distributor Details").SetFont(kabrioBoldFont).SetFontSize(10).SetPadding(0).SetTextAlignment(TextAlignment.CENTER)));
							...
							Table GstNumberTable = new Table(UnitValue.CreatePercentArray(new float[] { 40, 60 })).UseAllAvailableWidth();
							...
							if (!string.IsNullOrEmpty(data.PurchaseOrderDate))
							{
								GstNumberTable.AddCell(new Cell().SetBorder(Border.NO_BORDER).Add(new Paragraph("PO Date").SetFont(kabrioBoldFont).SetFontSize(9).SetTextAlignment(TextAlignment.LEFT)));
								GstNumberTable.AddCell(new Cell().SetBorder(Border.NO_BORDER).Add(new Paragraph().Add(new Text(": ").SetFont(kabrioBoldFont)).Add(new Text(data.PurchaseOrderDate).SetFont(kabrioFont)).SetFontSize(9).SetTextAlignment(TextAlignment.LEFT)));
							}

							if (!string.IsNullOrEmpty(data.ExpectedDeliveryDate))
							{
								GstNumberTable.AddCell(new Cell().SetBorder(Border.NO_BORDER).Add(new Paragraph("Exp Delivery Date").SetFont(kabrioBoldFont).SetFontSize(9).SetTextAlignment(TextAlignment.LEFT)));
								GstNumberTable.AddCell(new Cell().SetBorder(Border.NO_BORDER).Add(new Paragraph().Add(new Text(": ").SetFont(kabrioBoldFont)).Add(new Text(data.ExpectedDeliveryDate).SetFont(kabrioFont)).SetFontSize(9).SetTextAlignment(TextAlignment.LEFT)));
							}
							...
							else if (data.ProductItemTable.Columns.Count == 9)
							{
								...
								ProductTableColumnWidth = new float[] { 5, 20, 10, 10, 10, 15, 10, 10, 10 };
							}
							else if (data.ProductItemTable.Columns.Count == 10)
							{
								//for This Width will 10 Column
								ProductTableColumnWidth = new float[] { 5, 25, 10, 10, 5, 5, 5, 10, 10, 15 };
							}
							...
							for (int i = 0; i < data.ProductItemTable.Columns.Count; i++)
							{
								...
								Paragraph headerParagraph = new Paragraph().SetFont(kabrioBoldFont).SetFontSize(9).SetTextAlignment(TextAlignment.CENTER);
							...
							Cell mergedCell = new Cell(1, 5)
								.Add(new Paragraph("No Of Product : " + mergedText.Trim()).SetFont(kabrioBoldFont).SetFontSize(10)).SetBorderTop(new SolidBorder(1)).SetBorderBottom(Border.NO_BORDER).SetBorderLeft(new SolidBorder(1)).SetBorderRight(new SolidBorder(1)).SetTextAlignment(TextAlignment.CENTER).SetBackgroundColor(HeaderBackgroundColor);

							// Add the merged cell to the table
							ProductItemTable.AddCell(mergedCell);

							// Add the remaining columns dynamically
							for (int i = 5; i < data.ProductItemTable.Columns.Count; i++)
							{
								string totalValue = lastRow[i]?.ToString() ?? " ";

								Cell totalCell = new Cell()
									.Add(new Paragraph(totalValue).SetFont(kabrioBoldFont).SetFontSize(10)).SetBorderTop(new SolidBorder(1)).SetBorderBottom(Border.NO_BORDER).SetBorderLeft(new SolidBorder(1)).SetBorderRight(new SolidBorder(1)).SetTextAlignment(TextAlignment.CENTER).SetBackgroundColor(HeaderBackgroundColor);
							...
							RoundoffTable.AddCell(new Cell().SetBorder(Border.NO_BORDER).SetBorderBottom(new SolidBorder(0)).Add(new Paragraph("Grand Total").SetFont(kabrioBoldFont).SetFontSize(9).SetTextAlignment(TextAlignment.RIGHT)));
							...
							GrandTable.AddCell(new Cell().SetBorder(Border.NO_BORDER).SetBorderBottom(new SolidBorder(0)).Add(new Paragraph(data.GrantTotal).SetFont(kabrioBoldFont).SetFontSize(9).SetTextAlignment(TextAlignment.RIGHT)));
							...
							AmountFooterTable.AddCell(new Cell().SetBorderBottom(new SolidBorder(1)).SetBorderTop(Border.NO_BORDER).Add(new Paragraph(System.Globalization.CultureInfo.CurrentCulture.TextInfo.ToTitleCase(data.NumberToWords.ToLower()).Insert(0, "F").Substring(1)).SetFont(kabrioFont).SetFontSize(9).SetTextAlignment(TextAlignment.LEFT)));
							...
							NotesTable.AddCell(new Cell()
								...
								.Add(new Paragraph(notesContent)
								.SetFont(kabrioFont)
								.SetFontSize(9)
								.SetTextAlignment(TextAlignment.LEFT)));
							...
							TearmFooterTable.AddCell(new Cell(1, 3).SetBorder(Border.NO_BORDER)
								.Add(new Paragraph(data.TermsandConditions)
									.SetFont(kabrioFont).SetFontSize(7).SetTextAlignment(TextAlignment.LEFT)));
