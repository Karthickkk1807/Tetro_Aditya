...
$(document).ready(function(){
	var franchiseId = localStorage.getItem("FranchiseId");
	if (franchiseId === "undefined" || franchiseId === null || franchiseId === "null") {
		franchiseId = null;
	...
	bindDropDown(1, 'UserFranchiseMappingId', 'UserFranchiseMapping', function () {
		if (franchiseId) {
			...
			let newFranchiseId = $('#UserFranchiseMappingId').val();
			localStorage.setItem("FranchiseId", newFranchiseId);
		}
	});
	...
	if(!minimize_sidebar){
		...
		if($('.wrapper').hasClass('sidebar_minimize')){
			...
			minibutton.html('<i class=""><img src="/assets/commonimages/menubar.svg" /></i>');
		...
		minibutton.on('click', function() {
			if (mini_sidebar == 1) {
				...
				minibutton.html('<i class=""><img src="/assets/commonimages/menubar.svg" /></i>');
				mini_sidebar = 0;
				$('#skilift-logo1-show').css('display','none');
				$('#skilift-logo-show').css('display', 'block');
				...
				minibutton.html('<i class=""><img src="/assets/commonimages/menubar.svg" /></i>');
				mini_sidebar = 1;				
				$('#skilift-logo1-show').css('display','block');
				$('#skilift-logo-show').css('display', 'none');
	...
	if(!toggle_overlay_sidebar){
		...
		if($('.wrapper').hasClass('is-show')){
			...
			overlaybutton.html('<i class=""><img src="/assets/commonimages/menubar.svg" /></i>');
		}
	...
	$("#showButton").click(function (event) {
		...
		$('#buttonGroup1').toggleClass('d-none');
	});

	$(document).click(function (event) {
		if (!$(event.target).closest('#showButton').length) {
			$('#buttonGroup').addClass('d-none');
			$('#buttonGroup1').addClass('d-none');
		}
	});
	...
	$('input, select').on('input change', function () {
		// Remove 'is-invalid' and 'error' classes
		//$(this).removeClass('is-invalid error');
		// Hide the invalid-feedback
		$(this).next('.invalid-feedback').hide();
	});

	// When any input or select field loses focus (blur event)
	$('input, select').on('blur', function () {
		// If the input field is empty, show the invalid-feedback and add the 'is-invalid' and 'error' classes
		if ($(this).val().trim() === '' || ($(this).is('select') && $(this).prop('selectedIndex') === 0)) {
...
$(document).on('change','.up', function(){
	...
	var length = $(this).get(0).files.length;
		...
		if(length>2){
		  ...
		  $('.file-select-view').css('display','block');
	...
	$.ajax({
		...
		success: function (response) {
			if (response.status == true) {
				$('.backdrop').show();
				bindDropDownSuccess(response.data, id);
...
function bindDropDownSuccess(response, controlid) {
	if (response != null) {
...
$("#UserFranchiseMappingId").change(function () {

	localStorage.removeItem("FranchiseId");
	let newFranchiseId = $(this).val();
	localStorage.setItem("FranchiseId", newFranchiseId);
	location.reload();
});
